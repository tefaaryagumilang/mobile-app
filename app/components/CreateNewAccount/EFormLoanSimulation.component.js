...
import {map, noop, result, isEmpty, split} from 'lodash';
...
class EFormLoanSimulation extends Component {
  static propTypes = {
    page: PropTypes.func,
  ...
  sliderChange = (value) => {
    ...
    const period = result(formValues, '4~1~3.name', {});
    const periodValue = split(period, ' ');
    const totalInterest = (value * 0.7 / 100) * periodValue[0];
  ...
  render () {
    ...
    const {header, formName, fields = [], style} = page;
    const isNewApply = result(initialValues, 'isNewApply', true);
...
const ConnectedEFormSimulation = connect(loanSimulationState, loanSimulationDispatch)(EFormLoanSimulation);
