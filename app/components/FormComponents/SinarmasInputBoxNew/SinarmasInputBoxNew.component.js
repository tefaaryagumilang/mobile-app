...
import styles from './SinarmasInputBoxNew.styles';
...
class SinarmasInputBoxNew extends Component {
  _inputRef = null
  _setRef = (ref) => {
  ...
  render () {
    const {meta, input = {}, value, onFocus = noop, onBlur = noop, onChangeText = noop, borderOff, onInputChange = noop, disabled = false, isUseSuccessInputText = false, typeField = 'text', setNewUsernameValue, setNewPasswordValue, usernameValue = '', passwordValue = '', oldPassword = '', 
      validationInput, label = '', iconName = '', textStyle = {}, isQrTTS = false, isTax = false, KeyboardType, ...extraProps} = this.props;
    ...
    const borderStyle = borderOff ? {} : isQrTTS ? styles.containerInputBoxTTS : isTax ? styles.borderTax : styles.border;
    const labelStyleIOS = {
      position: 'absolute',
      left: 0,
    ...
    const rowStyle = extraProps.isInputAmount ? styles.rowAmount : styles.row;
    const disableStyle = extraProps.isInputAmount ? styles.inputStyleFocusAmount : isTax ? styles.inputStyleFocusTax : styles.inputStyleDisabled;
            ...
            { inputProps.value !== '' || this.state.isFocused ? 
              <View>
                ...
                <TextInput
                  ...
                  ref={this._setRef}
                  style={[textStyle, disabled ? disableStyle : isQrTTS ? styles.inputStyleFocusQr : isTax ? styles.inputStyleFocusTax : extraProps.isInputAmount ? styles.inputStyleFocusAmount : styles.inputStyleFocus]}
              ...
              <View>
                ...
                <TextInput
                  ...
                  textColor={disabled ? styles.tintColor.disabled : undefined}
                  ref={this._setRef}
                  style={[textStyle, disabled ? styles.inputStyleDisabled : isQrTTS ? styles.inputStyleFocusQr : styles.inputStyleFocus]}
                  onChangeText={this._onChangeTextHandler(inputProps)}
                  onFocus={this._onFocusHandler(inputProps)}
                  onBlur={this._onBlurHandler(inputProps, input.value)}
                  baseColor={disabled ? styles.tintColor.disabled : undefined}
...
SinarmasInputBoxNew.propTypes = {
  ...
  isQrTTS: PropTypes.bool,
  isTax: PropTypes.bool,
  KeyboardType: PropTypes.string
};

export default SinarmasInputBoxNew;