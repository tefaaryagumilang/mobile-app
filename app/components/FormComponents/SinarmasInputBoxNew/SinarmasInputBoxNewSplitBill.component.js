...
import {View, Animated, TextInput, Platform, Text} from 'react-native';
import noop from 'lodash/noop';
import styles from './SinarmasInputBoxNewSplitBill.styles';
...
class SinarmasInputBoxNewSplitBill extends Component {
  _inputRef = null
  _setRef = (ref) => {
  ...
  render () {
    const {meta, input = {}, value, onFocus = noop, onBlur = noop, onChangeText = noop, borderOff, onInputChange = noop, disabled = false, isUseSuccessInputText = false, typeField = 'text', setNewUsernameValue, setNewPasswordValue, usernameValue = '', passwordValue = '', oldPassword = '', 
      validationInput, label = '', iconName = '', isAmountSplitBill, placeholder = '', isSplitBillForm, ...extraProps} = this.props;
        ...
        <View>
          <View style={[styles.row, borderStyle]}>
            { inputProps.value !== '' || this.state.isFocused ? 
              <View>
                {isAmountSplitBill ?
                  <Text style={styles.labelAmount}>{label}</Text>
                  :
                ...
                <TextInput
                  ...
                  ref={this._setRef}
                  style={disabled && !isSplitBillForm ? styles.inputStyleDisabled : isAmountSplitBill ? styles.inputStyleFocusAmount : isSplitBillForm ? styles.inputStyleFocusNew : styles.inputStyleFocus}
                  ...
                  label={''}
                  placeholder={isAmountSplitBill ? placeholder : ''}
                  autoCapitalize={'none'}
                  disabled={disabled}
              ...
              <View>
                ...
                <TextInput
                  ...
                  textColor={disabled ? styles.tintColor.disabled : undefined}
                  ref={this._setRef}
                  style={disabled && !isSplitBillForm ? styles.inputStyleDisabled : isAmountSplitBill ? styles.inputStyleFocusAmount : isSplitBillForm ? styles.inputStyleFocusNew : styles.inputStyle}
                  onChangeText={this._onChangeTextHandler(inputProps)}
                  onFocus={this._onFocusHandler(inputProps)}
                  onBlur={this._onBlurHandler(inputProps, input.value)}
                  baseColor={disabled ? styles.tintColor.disabled : undefined}
...
SinarmasInputBoxNewSplitBill.propTypes = {
  ...
  isAmountSplitBill: PropTypes.bool,
  isSplitBillForm: PropTypes.bool,
};

export default SinarmasInputBoxNewSplitBill;
