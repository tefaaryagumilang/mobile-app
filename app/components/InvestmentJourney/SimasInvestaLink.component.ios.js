...
import {result, startCase, toLower, startsWith} from 'lodash';
...
class InvestmentComponent extends React.Component {
  static propTypes = {
    ...
    getCurrencyMultiSil: PropTypes.object
  };
  ...
  renderListPolis = ({item}) => {
    ...
    return (
      <View>
        <Touchable onPress={changeData(item)}>
          <View>
  ...
  renderInner = () => {
    ...
    return (
      <View style={styles.panel}>
        ...
        <FlatList
          data={listInfoPolis}
          renderItem={this.renderListPolis}
  ...
  _renderItem = ({data}) => {
    ...
    return (
      <View>
        <SimasInvestaLinkInfo
          emFundEnabled={emFundEnabled}
          infoPolis={infoPolis}
          goToEmFund={goToEmFund}
          summaryDetail={summaryDetail}
          getFlagEmergencyFund={getFlagEmergencyFund}
        />
      </View>
    );
  };
        ...
        >
          ...
          >
            <View style={styles.upperContainer}>
              <ImageBackground
                source={SILBackground}
                style={styles.backgroundImage}
              >
                <View style={styles.padding}>
                  <Text style={styles.uname}>
                    {startCase(toLower(polisHolderName))}
                  </Text>
                  {produkCodeFlag ? (
                    <Text style={styles.polis}>
                      {language.INQUIRY__SIL_NO_POLIS_NEW}
                    </Text>
                  ) : (
                    <Text style={styles.polis}>
                      {language.INQUIRY__SIL_NO_POLIS_OLD}
                    </Text>
                  )}
                  ...
                  <View style={styles.row}>
                    <View>
                      <Text style={styles.keys}>
                        {language.INQUIRY__SIL_UANG_PERTANGGUNGAN}
                      </Text>
                      <Text style={styles.keys}>
                        {language.INQUIRY__SIL_STATUS_POLIS}
                      </Text>
                    </View>
                    <View>
                      <Text style={styles.values}>
                        {mataUang}{' '}
                        {formatForexAmount(uangPertanggungan, mataUang)}
                      </Text>
                      <Text style={styles.values}>
                        {startCase(toLower(statusPolis))}
                      </Text>
                    </View>
                  </View>
                </View>
                {getFlagTopUP && (
                  <View>
                    <SinarmasButton
                      ...
                      onPress={this.goTopUpMenu}
                    >
                      <Text style={styles.topUpText}>
                        {language.INQUIRY__SIL_TOP_UP_BUTTONTEXT}
                      </Text>
                    </SinarmasButton>
                  </View>
                )}
            ...
            <View style={styles.lowerContainer}>
              <View>
                <Text style={styles.detailsTxt}>
                  {language.INQUIRY__SIL_DETAILS}
                </Text>
                <Carousel
                  data={listDetailSummary}
                ...
                <View>
                  {produkCodeFlag ? (
                    <Text style={styles.textFooter2}>
                      {language.INQUIRY__SIL_FOOTER_2_NEW}
                    </Text>
                  ) : (
                    <Text style={styles.textFooter2}>
                      {language.INQUIRY__SIL_FOOTER_2_OLD}
                    </Text>
                  )}
                </View>
              </View>
