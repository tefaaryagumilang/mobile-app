...
class OTPFormView extends Component {
  ...
  render () {
    const {currentLanguage, dontRecogniseNumber, newUserMobileMigrate, isMigrate, dtRegisterAtm, ...reduxFormProps} = this.props;
    ...
    return (
      <KeyboardAwareScrollView contentContainerStyle={styles.bodyContainerOTP} extraHeight={120}>
        ...
        <View style={styles.buttonOtpSubmit}>
          <SinarmasButtonOnboarding dtActionName={dtRegisterAtm ? dtRegisterAtm + ' - Input SMS OTP' : 'SMS OTP'} onPress={wrapMethodInFunction(handleSubmit)} disabled={invalid || submitting} style={styles.buttonOtpSubmitSimas} >
...
OTPFormView.propTypes = {
  ...
  dtRegisterAtm: PropTypes.string
};
