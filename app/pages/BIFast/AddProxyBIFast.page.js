...
import FundTransferPayment, {fields} from '../../components/BIFast/AddProxyBIFast.component';
...
import {goToConfirmationParams} from '../../state/thunks/biFast.thunks';
...
class FundTransferPaymentPage extends Component {
  ...
  confirmationGoFunc = () => {
    const {formValues, goToConfirmation, addProxyAddress, proxyAlias} = this.props;
    // const isEmail = result(detailByCustNo, 'responseGetDetailByCustNo.body.response.emailAddress', '');
    // const isPhoneNumber = result(detailByCustNo, 'responseGetDetailByCustNo.body.response.phoneNumber', '');
    const myAccount = result(formValues, 'myAccount', {});
    goToConfirmation(addProxyAddress, proxyAlias, myAccount);
  }
  ...
  selectSourceAcc = () => {
    ...
    dispatch(change('addProxyAddress', 'currency', currencyTarget));
  ...
  render () {
    const {navigation, accountList, formValues, appConfig, unregisteredName, getCurrency, goBack, user, detailByCustNo, addProxyAddress, prefiledAliasProxy, proxyAlias} = this.props;
    const isEmail = result(detailByCustNo, 'responseGetDetailByCustNo.body.response.emailAddress', '');
    const isPhoneNumber = result(detailByCustNo, 'responseGetDetailByCustNo.body.response.phoneNumber', '');
    const myAccount = result(formValues, 'myAccount', {});
    ...
    return (
      <DecoratedFundTransferPayment accountList={accounts} onNextPress={this.onNext} payee={payee}
        ...
        currencyTarget={currencyTarget} isValas={isValas} amountNormalise={this.amountNormalise} getCurrency={getCurrency} goBack={goBack} 
        visible={this.state.visible}
        tickOverlay={this.tickOverlay}
        tickOnclose={this.tickOnclose} 
        user={user} detailByCustNo={detailByCustNo} isEmail={isEmail} isPhoneNumber={isPhoneNumber} myAccount={myAccount} goToConfirmation={this.confirmationGoFunc}
        prefiledAliasProxy={prefiledAliasProxy} addProxyAddress={addProxyAddress} proxyAlias={proxyAlias}/>
    );
  }
}
...
const mapStateToProps = (state) => ({
  ...
  formValues: result(state, 'form.addProxyAddress.values', {}),
  ...
  addProxyAddress: result(state, 'form.addProxyAddress.values.addProxyType.label', ''),
  proxyAlias: result(state, 'form.addProxyAddress.values.proxyAlias', ''),
});
...
const mapDispatchToProps = (dispatch) => ({
  ...
  goToConfirmation: (addProxyAddress, proxyAlias, myAccount) => {
    dispatch(goToConfirmationParams(addProxyAddress, proxyAlias, myAccount));
  },
  ...
  getSourceAcc: (payee, isTopup, isValas) => {
    dispatch(NavigationActions.navigate({
      ...
      params: {formName: 'addProxyAddress', fieldName: 'myAccount', sourceType: 'fundTransfer', payee: payee, isTopup: isTopup, isValas: isValas}
    }));
  },
  ...
  prefiledAliasProxy: (aliasProxy) => {
    dispatch(change('addProxyAddress', 'proxyAlias', aliasProxy));
  },
  dispatch
});
