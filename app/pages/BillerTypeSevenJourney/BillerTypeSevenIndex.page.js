...
import BillerTypeSevenIndex from '../../components/BillerTypeSevenJourney/BillerTypeSevenIndex.component';
import {reduxForm, change} from 'redux-form';
import {connect} from 'react-redux';
import {getAmountForGenericBillerTypeSeven} from '../../state/thunks/genericBill.thunks';
import {
...
const mapDispatchToProps = (dispatch) => ({
  ...
  selectHistory: (subscriberNo) => {
    dispatch(change('BillerTypeSevenIndexForm', 'subscriberNo', subscriberNo));
  },
  getBillpayHistory: () => dispatch(getBillpayHistory()),
});

const formConfig = {
  form: 'BillerTypeSevenIndexForm',
  validate: (values, {navigation}) => {
    ...
    const subscriberNoText = result(
      biller,
      'billerPreferences.purchaseSubscriberNoKey',
      ''
    );
    const errors = {
  ...
  onSubmit: (values, dispatch, {navigation}) => {
    ...
    dispatch(
      getAmountForGenericBillerTypeSeven(values.subscriberNo, biller, favBiller)
    );
  }
};

const BillerTypeSevenIndexForm = reduxForm(formConfig)(BillerTypeSevenIndex);

class BillerTypeSevenIndexPage extends Component {
  static propTypes = {
...
const ConnectedBillerTypeSevenIndexPage = connect(
  mapStateToProps,
  mapDispatchToProps
)(BillerTypeSevenIndexPage);
export default ConnectedBillerTypeSevenIndexPage;
