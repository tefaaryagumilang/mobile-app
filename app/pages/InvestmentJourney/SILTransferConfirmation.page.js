...
import FundTransferConfirmation from '../../components/InvestmentJourney/SILTransferConfirmation.component';
...
class FundTransferConfirmationPage extends Component {
  ...
  render () {
    const {navigation, triggerAuth, appConfig, gapTimeServer, favoriteBill, billerFavorite, showFavorite, removeFavorite, ownAccount, saveAlias, inquirySIL, currencySil} = this.props;
    const payee = result(navigation, 'state.params.payee', {});
    const formValues = result(navigation, 'state.params.formValues', {});
    ...
    return <DecoratedFundTransferConfirmation
      ...
      ownAccount={ownAccount} isFavorite={isFavorite} saveAlias={saveAlias} inquirySIL={inquirySIL}
      currencySil={currencySil} amount={amount} currencyIcon={currencyIcon}
    />;
  }
}
...
const mapDispatchToProps = (dispatch) => ({
  ...
  transfer: (values, payee, resData, infoPolis) => dispatch(transferSIL(values, payee, 'topUpSil', resData, infoPolis)),
