...
import SplitBillIndex, {fields} from '../../components/SplitBillJourney/SplitBillIndex.component.js';
...
import {confirmSplitBill, goToYouBill, goToSplitBill} from '../../state/thunks/splitBill.thunks';
...
const formConfig = {
  form: 'SplitBillIndexForm1',
  destroyOnUnmount: false,
  onSubmit: (values, dispatch, {selectedContacts, navConfirmation, dataTrx, validateStatus}) => {
    if (validateStatus) {
      ...
      dispatch(confirmSplitBill(selectedContacts, navConfirmation, validateMember));
    }
  },
};
...
const mapStateToProps = (state) => ({
  ...
  selectedContacts: isEmpty(result(state, 'selectedContacts', [])) ? result(state, 'saveListSelectedContacts', []) : result(state, 'selectedContacts', []),
  addNewParticipantsValue: result(state, 'saveListSelectedContacts', []),
  addContact: result(state, 'addContact', []),
  form: result(state, 'form.SplitBillIndexForm1.values'),
...
const mapDispatchToProps = (dispatch) => ({
  ...
  dispatch,
  confirmSplitBillAuth: (selectedContact) => dispatch(confirmSplitBill(selectedContact)),
  ...
  setPhone: (name, phone) => {
    dispatch(change('SplitBillIndexForm1', fields.PHONE, phone));
  },
  setAlias: (name, givenName) => {
    dispatch(change('SplitBillIndexForm1', fields.ALIAS, givenName));
  },
});

const SplitBillIndexForm = reduxForm(formConfig)(SplitBillIndex);

class SplitBillIndexPage extends React.Component {
  ...
  static propTypes = {
    ...
    addNewParticipantsValue: PropTypes.object,
  ...
  selectedContact = () => {
    ...
    dispatch(actionCreators.saveSelectedContacts(finallSelected));
    ...
    if (size(filterContact) === 0) {
      ...
      if (formatMobileNumberEmoney(phoneNumber) !== formatMobileNumberEmoney(userMobileNumber)) {
        if (!validateDuplicate) {
          ...
          dispatch(actionCreators.saveSelectedContacts(selected));
          setTimeout(() => {
  ...
  selectedContactdelete = () => {
    const {contacts, dispatch, selectedContacts, addNewParticipantsValue} = this.props;
    // identity route create or add new participants
    ...
    // process deleting
    let selected = [];
    map(sortingFilterAddNew, (contact) => {
  ...
  updateSelectedContacts = (newContacts = []) => {
    ...
    const {dispatch} = this.props;
    dispatch(actionCreators.saveSelectedContacts(newContacts));
  }
  componentDidMount () {
    const {dispatch, addNewParticipantsValue} = this.props;
    if (!isEmpty(addNewParticipantsValue)) {
      dispatch(actionCreators.saveSelectedContacts(addNewParticipantsValue));
    }
  }
  
  render () {
    const {phone, givenName} = this.state;
    const {accounts, formValues, navigation, contacts, selectedContacts, form, getListSender, addNewParticipantsValue, dispatch} = this.props;
    ...
    return <SplitBillIndexForm navConfirmation={navConfirmation} contacts={sortingContact} selectedContacts={sortingSelectContact} 
      ...
      validateStatus={validateStatus} paramsSelectedContacts={paramsSelectedContacts} isAddNewParticipants={isAddNewParticipants} addNewParticipantsValue={addNewParticipantsValue}
      validationInput={this.validationInput} dispatch={dispatch}
    />;
  }
}

export default connect(mapStateToProps, mapDispatchToProps)(SplitBillIndexPage);
